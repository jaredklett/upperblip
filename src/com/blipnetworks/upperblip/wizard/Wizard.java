 ...
 *
 * $Id: Wizard.java,v 1.2 2009/06/22 21:21:54 jklett Exp $
 */
 ...
 * then call {@link Wizard#showInFrame} or one of the related methods.
 * <p>
...
public class Wizard extends JPanel {
	...
	public Wizard(WizardModel model) {
		...
		if (model instanceof OverviewProvider) {
			overviewContainer.add(((OverviewProvider) model).getOverviewComponent(), BorderLayout.CENTER);
			add(overviewContainer, BorderLayout.WEST);
		}
		
		JPanel	rightSide = new JPanel(new BorderLayout());
		titleComponent = createTitleComponent();
		rightSide.add(titleComponent, BorderLayout.NORTH);
		
		buttonBar = createButtonBar();
		rightSide.add(buttonBar, BorderLayout.SOUTH);

		viewPanel = new JPanel(new BorderLayout());
		rightSide.add(viewPanel, BorderLayout.CENTER);
	 ...
	 * this will be an instance of {@link DefaultTitleComponent}. Subclasses can
	 * change the default by overriding {@link #createTitleComponent()}.
	 * <p>
	...
	/**
	 * Checks the visibility of the overview panel that is displayed on the
	 * wizards left panel. The overview panel will only be displayed if this
	 * property is <tt>true</tt> and the current {@link WizardModel} implements
	...
	/**
	 * Configures the visibility of the overview panel that is displayed on the
	...
	/**
	 * Calculates the preferred size of the main wizard area based on the
	...
	/*private Dimension calculatePreferredStepSize() {
		...
		for (WizardStep step : getModel().getSteps()) {
	 ...
	 *            component. If the component is <tt>null</tt>, the window will
	 *            be centered on the desktop as per
	 ...
	 *            <tt>null</tt>, the window will be centered on the desktop as
	 *            per {@link JWindow#setLocationRelativeTo(java.awt.Component)}.
	 */
	private void showInWindow(JFrame window, Component relativeTo) {
		System.out.println("showInWindow");
